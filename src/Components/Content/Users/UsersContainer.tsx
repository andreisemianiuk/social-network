...
class UsersContainer extends React.Component<UsersPagePropsType> {
  componentDidMount() {
    ...
    axios.get<GetUsersResponseType>(`https://social-network.samuraijs.com/api/1.0/users`,{withCredentials: true}).then(response => {
  ...
  changeCurrentPage = (page: number) => {
    ...
    axios.get<GetUsersResponseType>(
      `https://social-network.samuraijs.com/api/1.0/users?page=${page}&count=${this.props.pageSize}`, {withCredentials: true},
    ).then(response => {
      this.props.toggleFetching(false)
      this.props.setUsers(response.data.items)
    })
